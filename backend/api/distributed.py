...
PROBLEM_TYPE_CONFIG = {
    ...
    "token_bucket": {
        "directory": "token_bucket",
        "proto_file": "token_bucket.proto",
        "java_class": "TokenBucketServer",
        "test_runner": "backend.services.distributed_tests_rate_limiter.TokenBucketTestRunner",
    },
    "leaky_bucket": {
        "directory": "leaky_bucket",
        "proto_file": "leaky_bucket.proto",
        "java_class": "LeakyBucketServer",
        "test_runner": "backend.services.distributed_tests_rate_limiter.LeakyBucketTestRunner",
    },
    "fixed_window_counter": {
        "directory": "fixed_window_counter",
        "proto_file": "fixed_window_counter.proto",
        "java_class": "FixedWindowCounterServer",
        "test_runner": "backend.services.distributed_tests_rate_limiter.FixedWindowTestRunner",
    },
    "sliding_window_log": {
        "directory": "sliding_window_log",
        "proto_file": "sliding_window_log.proto",
        "java_class": "SlidingWindowLogServer",
        "test_runner": "backend.services.distributed_tests_rate_limiter.SlidingWindowLogTestRunner",
    },
    "sliding_window_counter": {
        "directory": "sliding_window_counter",
        "proto_file": "sliding_window_counter.proto",
        "java_class": "SlidingWindowCounterServer",
        "test_runner": "backend.services.distributed_tests_rate_limiter.SlidingWindowCounterTestRunner",
    },
    "three_phase_commit": {
        "directory": "three_phase_commit",
        "proto_file": "three_phase_commit.proto",
        "java_class": "ThreePhaseCommitServer",
        "test_runner": "backend.services.distributed_tests_3pc.ThreePhaseCommitTestRunner",
    },
    "crdt": {
        "directory": "crdt",
        "proto_file": "crdt.proto",
        "java_class": "CRDTServer",
        "test_runner": "backend.services.distributed_tests_crdt.CRDTTestRunner",
    },
    "kv_store_log": {
        "directory": "kv_store_log",
        "proto_file": "kv_store_log.proto",
        "java_class": "LogStructuredKVServer",
        "test_runner": "backend.services.distributed_tests_kv.LogStructuredKVTestRunner",
    },
    "kv_store_lsm": {
        "directory": "kv_store_lsm",
        "proto_file": "kv_store_lsm.proto",
        "java_class": "LSMTreeKVServer",
        "test_runner": "backend.services.distributed_tests_kv.LSMTreeTestRunner",
    },
    "kv_store_btree": {
        "directory": "kv_store_btree",
        "proto_file": "kv_store_btree.proto",
...
DEFAULT_PROBLEM_IDS = {
    ...
    7: "token_bucket",
    8: "leaky_bucket",
    9: "fixed_window_counter",
    10: "sliding_window_log",
    11: "sliding_window_counter",
    12: "three_phase_commit",
    13: "crdt",
    14: "kv_store_log",
    15: "kv_store_lsm",
    16: "kv_store_btree",
    ...
    if not problems:
        return [
            ...
            },
            {
                ...
                "description": "Implement Hierarchical Navigable Small World (HNSW) graph for approximate nearest neighbor search.",
                "difficulty": "hard",
                "problem_type": "distributed_consensus",
                "supported_languages": ["python", "go", "java", "cpp", "rust"],
                "cluster_size": 3,
                "tags": ["distributed-systems", "vector-search", "hnsw", "ann"],
                "created_at": "2025-01-01T00:00:00Z",
            },
            {
                "id": 18,
                "title": "Implement Inverted File Index (IVF)",
                "description": "Implement Inverted File Index for efficient vector similarity search using clustering.",
                "difficulty": "hard",
                "problem_type": "distributed_consensus",
                "supported_languages": ["python", "go", "java", "cpp", "rust"],
                "cluster_size": 3,
                "tags": ["distributed-systems", "vector-search", "ivf", "clustering"],
                "created_at": "2025-01-01T00:00:00Z",
            },
            {
                "id": 19,
                "title": "Implement Product Quantization (PQ)",
                "description": "Implement Product Quantization for vector compression and fast similarity search.",
                "difficulty": "hard",
                "problem_type": "distributed_consensus",
                "supported_languages": ["python", "go", "java", "cpp", "rust"],
                "cluster_size": 3,
                "tags": ["distributed-systems", "vector-search", "pq", "compression"],
                "created_at": "2025-01-01T00:00:00Z",
            },
        ]
...
def _get_default_problem(problem_id: int, problem_type: str) -> dict:
    ...
    problems_config = {
        ...
        "token_bucket": {
            "id": 7,
            "title": "Implement Token Bucket Rate Limiter",
            "description": "Implement a distributed token bucket rate limiter.\n\nKey concepts:\n1. Bucket Capacity - Maximum number of tokens\n2. Refill Rate - Tokens added per second\n3. Token Consumption - Each request consumes tokens\n4. Distributed State - Synchronize bucket state across nodes\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "medium",
            "test_scenarios": [
                {"name": "Basic Allow", "description": "Verify requests are allowed when tokens available", "test_type": "functional"},
                {"name": "Basic Deny", "description": "Verify requests are denied when no tokens", "test_type": "functional"},
                {"name": "Token Refill", "description": "Verify tokens refill over time", "test_type": "functional"},
                {"name": "Burst Handling", "description": "Verify burst requests up to capacity", "test_type": "functional"},
                {"name": "Distributed Sync", "description": "Verify state sync across nodes", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Throughput", "description": "Measure rate limiter throughput", "test_type": "performance"},
            ],
            "hints": [
                "Track last refill time to calculate tokens to add",
                "Use atomic operations for thread safety",
                "Consider lazy refill on each request",
                "Handle time drift between nodes",
            ],
            "tags": ["distributed-systems", "rate-limiting", "token-bucket"],
        },
        "leaky_bucket": {
            "id": 8,
            "title": "Implement Leaky Bucket Rate Limiter",
            "description": "Implement a distributed leaky bucket rate limiter.\n\nKey concepts:\n1. Queue Capacity - Maximum requests in queue\n2. Leak Rate - Requests processed per second\n3. Queue Management - FIFO request processing\n4. Overflow Handling - Reject when queue full\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "medium",
            "test_scenarios": [
                {"name": "Basic Allow", "description": "Verify requests are queued when space available", "test_type": "functional"},
                {"name": "Queue Overflow", "description": "Verify rejection when queue full", "test_type": "functional"},
                {"name": "Leak Rate", "description": "Verify constant processing rate", "test_type": "functional"},
                {"name": "Smoothing", "description": "Verify burst traffic is smoothed", "test_type": "functional"},
                {"name": "Distributed Sync", "description": "Verify state sync across nodes", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Throughput", "description": "Measure rate limiter throughput", "test_type": "performance"},
            ],
            "hints": [
                "Use a queue data structure for pending requests",
                "Process requests at fixed intervals",
                "Track queue size for overflow detection",
                "Consider using a timer for leak processing",
            ],
            "tags": ["distributed-systems", "rate-limiting", "leaky-bucket"],
        },
        "fixed_window_counter": {
            "id": 9,
            "title": "Implement Fixed Window Counter Rate Limiter",
            "description": "Implement a distributed fixed window counter rate limiter.\n\nKey concepts:\n1. Window Size - Fixed time period (e.g., 1 minute)\n2. Request Limit - Maximum requests per window\n3. Counter Reset - Reset at window boundary\n4. Distributed Counting - Sync counts across nodes\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "medium",
            "test_scenarios": [
                {"name": "Basic Allow", "description": "Verify requests allowed under limit", "test_type": "functional"},
                {"name": "Limit Exceeded", "description": "Verify rejection when limit exceeded", "test_type": "functional"},
                {"name": "Window Reset", "description": "Verify counter resets at window boundary", "test_type": "functional"},
                {"name": "Boundary Burst", "description": "Test burst at window boundaries", "test_type": "functional"},
                {"name": "Distributed Sync", "description": "Verify state sync across nodes", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Throughput", "description": "Measure rate limiter throughput", "test_type": "performance"},
            ],
            "hints": [
                ...
                "Handle edge case of requests at window boundary",
        ...
        "sliding_window_log": {
            "id": 10,
            "title": "Implement Sliding Window Log Rate Limiter",
            "description": "Implement a distributed sliding window log rate limiter.\n\nKey concepts:\n1. Request Log - Store timestamp of each request\n2. Window Slide - Count requests in last N seconds\n3. Log Cleanup - Remove old entries\n4. Distributed Log - Sync logs across nodes\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "medium",
            "test_scenarios": [
                {"name": "Basic Allow", "description": "Verify requests allowed under limit", "test_type": "functional"},
                {"name": "Limit Exceeded", "description": "Verify rejection when limit exceeded", "test_type": "functional"},
                {"name": "Window Slide", "description": "Verify old requests fall out of window", "test_type": "functional"},
                {"name": "Precise Counting", "description": "Verify accurate count in sliding window", "test_type": "functional"},
                {"name": "Distributed Sync", "description": "Verify state sync across nodes", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Memory Usage", "description": "Test memory under high load", "test_type": "performance"},
            ],
            "hints": [
                "Store timestamps in a sorted data structure",
                "Remove timestamps older than window size",
                "Use binary search for efficient cleanup",
                "Consider memory usage for high-volume keys",
            ],
            "tags": ["distributed-systems", "rate-limiting", "sliding-window"],
        },
        "sliding_window_counter": {
            "id": 11,
            "title": "Implement Sliding Window Counter Rate Limiter",
            "description": "Implement a distributed sliding window counter rate limiter.\n\nKey concepts:\n1. Current Window - Count in current time window\n2. Previous Window - Count in previous window\n3. Weighted Average - Combine windows based on position\n4. Memory Efficient - Only store two counters\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "medium",
            "test_scenarios": [
                {"name": "Basic Allow", "description": "Verify requests allowed under limit", "test_type": "functional"},
                {"name": "Limit Exceeded", "description": "Verify rejection when limit exceeded", "test_type": "functional"},
                {"name": "Weighted Count", "description": "Verify correct weighted calculation", "test_type": "functional"},
                {"name": "Window Transition", "description": "Test smooth transition between windows", "test_type": "functional"},
                {"name": "Distributed Sync", "description": "Verify state sync across nodes", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Throughput", "description": "Measure rate limiter throughput", "test_type": "performance"},
            ],
            "hints": [
                ...
                "Estimated count = prev_count * weight + curr_count",
                "Only need to store prev_count, curr_count, and window_start",
                "Rotate windows when current window expires",
            ],
            "tags": ["distributed-systems", "rate-limiting", "sliding-window"],
        },
        "three_phase_commit": {
            "id": 12,
            "title": "Implement Three-Phase Commit",
            "description": "Implement the Three-Phase Commit (3PC) protocol for distributed transactions.\n\nKey concepts:\n1. CanCommit Phase - Coordinator asks if participants can commit\n2. PreCommit Phase - If all agree, prepare to commit\n3. DoCommit Phase - Actually commit the transaction\n4. Timeout Handling - Non-blocking recovery from failures\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "hard",
            "test_scenarios": [
                {"name": "Successful Commit", "description": "Verify 3-phase commit succeeds", "test_type": "functional"},
                {"name": "Abort on CanCommit", "description": "Verify abort if participant says no", "test_type": "functional"},
                {"name": "PreCommit State", "description": "Verify correct PreCommit behavior", "test_type": "functional"},
                {"name": "Coordinator Failure", "description": "Test recovery from coordinator failure", "test_type": "chaos"},
                {"name": "Participant Failure", "description": "Test recovery from participant failure", "test_type": "chaos"},
                {"name": "Network Partition", "description": "Test behavior during partition", "test_type": "chaos"},
                {"name": "Transaction Latency", "description": "Measure 3PC latency", "test_type": "performance"},
            ],
            "hints": [
                "3PC adds PreCommit phase between voting and commit",
                "PreCommit ensures all participants know the decision",
                "Timeout in PreCommit state leads to abort",
                "Timeout in DoCommit state leads to commit",
        ...
        "crdt": {
            "id": 13,
            "title": "Implement CRDTs",
            "description": "Implement Conflict-free Replicated Data Types (CRDTs).\n\nKey types to implement:\n1. G-Counter - Grow-only counter\n2. PN-Counter - Increment/decrement counter\n3. G-Set - Grow-only set\n4. LWW-Register - Last-writer-wins register\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "hard",
            "test_scenarios": [
                {"name": "G-Counter Increment", "description": "Verify G-Counter increments correctly", "test_type": "functional"},
                {"name": "G-Counter Merge", "description": "Verify G-Counter merges correctly", "test_type": "functional"},
                {"name": "PN-Counter", "description": "Verify PN-Counter works", "test_type": "functional"},
                {"name": "G-Set Add", "description": "Verify G-Set add and merge", "test_type": "functional"},
                {"name": "LWW-Register", "description": "Verify LWW-Register behavior", "test_type": "functional"},
                {"name": "Convergence", "description": "Verify all replicas converge", "test_type": "functional"},
                {"name": "Concurrent Updates", "description": "Test concurrent updates", "test_type": "chaos"},
                {"name": "Replication Latency", "description": "Measure sync latency", "test_type": "performance"},
            ],
            "hints": [
                "G-Counter: each node has its own counter, merge takes max",
                "PN-Counter: two G-Counters (positive and negative)",
                "G-Set: union for merge, no remove operation",
                "LWW-Register: highest timestamp wins",
                "All CRDTs must be commutative, associative, and idempotent",
            ],
            "tags": ["distributed-systems", "eventual-consistency", "crdt"],
        },
        "kv_store_log": {
            "id": 14,
            "title": "Implement Log-Structured KV Store",
            "description": "Implement a distributed KV store with append-only log storage.\n\nKey concepts:\n1. Append-Only Log - All writes append to log file\n2. In-Memory Index - KeyDir maps keys to file positions\n3. Compaction - Merge and remove stale entries\n4. Crash Recovery - Rebuild index from log\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "hard",
            "test_scenarios": [
                {"name": "Put/Get", "description": "Verify basic put and get", "test_type": "functional"},
                {"name": "Delete", "description": "Verify delete with tombstone", "test_type": "functional"},
                {"name": "Compaction", "description": "Verify compaction removes stale entries", "test_type": "functional"},
                {"name": "Crash Recovery", "description": "Verify recovery from crash", "test_type": "functional"},
                {"name": "Segment Rotation", "description": "Verify segment rotation", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Write Throughput", "description": "Measure write throughput", "test_type": "performance"},
                {"name": "Read Latency", "description": "Measure read latency", "test_type": "performance"},
            ],
            "hints": [
                "Each log entry: CRC + timestamp + key_size + value_size + key + value",
                "KeyDir stores: (segment_id, offset, size, timestamp)",
                "Tombstone: special value marker for deletes",
                "Compaction: write live keys to new segment, delete old segments",
            ],
            "tags": ["distributed-systems", "storage", "kv-store", "log-structured"],
        },
        "kv_store_lsm": {
            "id": 15,
            "title": "Implement LSM Tree KV Store",
            "description": "Implement a distributed KV store backed by LSM Tree.\n\nKey concepts:\n1. MemTable - In-memory sorted structure for writes\n2. WAL - Write-Ahead Log for durability\n3. SSTable - Sorted String Table for persistent storage\n4. Compaction - Merge SSTables across levels\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "hard",
            "test_scenarios": [
                {"name": "Put/Get", "description": "Verify basic put and get", "test_type": "functional"},
                {"name": "Delete", "description": "Verify delete with tombstone", "test_type": "functional"},
                {"name": "MemTable Flush", "description": "Verify MemTable flushes to SSTable", "test_type": "functional"},
                {"name": "WAL Recovery", "description": "Verify recovery from WAL", "test_type": "functional"},
                {"name": "Compaction", "description": "Verify level compaction", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Write Throughput", "description": "Measure write throughput", "test_type": "performance"},
                {"name": "Read Latency", "description": "Measure read latency", "test_type": "performance"},
            ],
            "hints": [
                ...
                "Read path: MemTable -> Immutable MemTable -> L0 -> L1 -> ...",
                "Use bloom filters to skip SSTables without key",
                "Compaction merges overlapping SSTables to next level",
            ],
            "tags": ["distributed-systems", "storage", "kv-store", "lsm-tree"],
        },
        "kv_store_btree": {
            "id": 16,
            "title": "Implement B+ Tree KV Store",
            "description": "Implement a distributed KV store backed by B+ Tree.\n\nKey concepts:\n1. B+ Tree - Balanced tree with all values in leaf nodes\n2. Pages - Fixed-size disk blocks for storage\n3. Buffer Pool - In-memory page cache\n4. WAL - Write-Ahead Logging for transactions\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "hard",
            "test_scenarios": [
                {"name": "Put/Get", "description": "Verify basic put and get", "test_type": "functional"},
                {"name": "Delete", "description": "Verify delete operation", "test_type": "functional"},
                {"name": "Node Split", "description": "Verify B+ Tree node splitting", "test_type": "functional"},
                {"name": "Range Scan", "description": "Verify range queries via leaf links", "test_type": "functional"},
                {"name": "Buffer Pool", "description": "Verify page caching", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Read Latency", "description": "Measure read latency", "test_type": "performance"},
                {"name": "Range Query Performance", "description": "Measure range query performance", "test_type": "performance"},
            ],
            "hints": [
                "Internal nodes store keys and child pointers",
                "Leaf nodes store keys and values, linked together",
                "Split when node is full, merge when underflow",
                "Buffer pool uses LRU for page eviction",
                "WAL ensures durability before page writes",
            ],
            "tags": ["distributed-systems", "storage", "kv-store", "b-plus-tree"],
        },
        "hnsw": {
            "id": 17,
            "title": "Implement HNSW Index",
            "description": "Implement Hierarchical Navigable Small World (HNSW) graph for approximate nearest neighbor search.\n\nKey concepts:\n1. Hierarchical Layers - Multiple graph layers with decreasing density\n2. Small World Graph - Navigable graph structure at each layer\n3. Insert Algorithm - Find neighbors at each layer, connect with edges\n4. Search Algorithm - Navigate from top layer to bottom, refine results\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "hard",
            "test_scenarios": [
                {"name": "Index Creation", "description": "Verify HNSW index is created with correct parameters", "test_type": "functional"},
                {"name": "Vector Insert", "description": "Verify vectors are inserted correctly", "test_type": "functional"},
                {"name": "K-NN Search", "description": "Verify k-nearest neighbor search returns correct results", "test_type": "functional"},
                {"name": "Recall Accuracy", "description": "Verify search recall meets quality threshold", "test_type": "functional"},
                {"name": "Layer Navigation", "description": "Verify multi-layer graph traversal works correctly", "test_type": "functional"},
                {"name": "Vector Delete", "description": "Verify vectors can be deleted from index", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Search Latency", "description": "Measure search latency under load", "test_type": "performance"},
                {"name": "Insert Throughput", "description": "Measure insert throughput", "test_type": "performance"},
            ],
            "hints": [
                "Use M parameter to control max connections per node",
                "efConstruction controls search quality during index building",
                "Entry point is always at the highest layer",
                "Lower layers have more nodes and connections",
                "Use greedy search to find nearest neighbors at each layer",
            ],
            "tags": ["distributed-systems", "vector-search", "hnsw", "ann"],
        },
        "inverted_file_index": {
            "id": 18,
            "title": "Implement Inverted File Index (IVF)",
            "description": "Implement Inverted File Index for efficient vector similarity search using clustering.\n\nKey concepts:\n1. Coarse Quantizer - K-means clustering to partition vector space\n2. Inverted Lists - Store vectors in cluster buckets\n3. Probe Search - Search nearest nprobe clusters\n4. Training - Learn cluster centroids from sample data\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "hard",
            "test_scenarios": [
                {"name": "Index Training", "description": "Verify IVF index training with k-means clustering", "test_type": "functional"},
                {"name": "Vector Add", "description": "Verify vectors are assigned to correct clusters", "test_type": "functional"},
                {"name": "Cluster Search", "description": "Verify search probes correct number of clusters", "test_type": "functional"},
                {"name": "Recall vs nprobe", "description": "Verify recall improves with more probes", "test_type": "functional"},
                {"name": "Centroid Quality", "description": "Verify cluster centroids represent data well", "test_type": "functional"},
                {"name": "Empty Clusters", "description": "Handle edge case of empty clusters", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Search Latency", "description": "Measure search latency vs nprobe", "test_type": "performance"},
                {"name": "Training Time", "description": "Measure index training time", "test_type": "performance"},
            ],
            "hints": [
                "Use k-means or k-means++ for clustering",
                "nlist parameter controls number of clusters",
                "nprobe parameter controls search accuracy vs speed tradeoff",
                "Store inverted lists as posting lists with vector IDs",
                "Consider using PQ for compressed storage within clusters",
            ],
            "tags": ["distributed-systems", "vector-search", "ivf", "clustering"],
        },
        "product_quantization": {
            "id": 19,
            "title": "Implement Product Quantization (PQ)",
            "description": "Implement Product Quantization for vector compression and fast similarity search.\n\nKey concepts:\n1. Subvector Division - Split vectors into M subvectors\n2. Codebook Training - Learn K centroids per subvector\n3. Encoding - Map each subvector to nearest centroid ID\n4. ADC Search - Asymmetric Distance Computation using lookup tables\n\nRefer to the gRPC proto file for the required service interfaces.",
            "difficulty": "hard",
            "test_scenarios": [
                {"name": "Codebook Training", "description": "Verify codebook training produces valid centroids", "test_type": "functional"},
                {"name": "Vector Encoding", "description": "Verify vectors are encoded to correct codes", "test_type": "functional"},
                {"name": "Vector Decoding", "description": "Verify approximate vector reconstruction", "test_type": "functional"},
                {"name": "Distance Computation", "description": "Verify ADC distance approximates true distance", "test_type": "functional"},
                {"name": "Compression Ratio", "description": "Verify expected compression ratio achieved", "test_type": "functional"},
                {"name": "Search Quality", "description": "Verify search recall meets threshold", "test_type": "functional"},
                {"name": "Node Failure", "description": "Test behavior during node failure", "test_type": "chaos"},
                {"name": "Search Latency", "description": "Measure search latency with PQ", "test_type": "performance"},
                {"name": "Encoding Throughput", "description": "Measure vector encoding throughput", "test_type": "performance"},
            ],
            "hints": [
                "M parameter controls number of subvectors (compression level)",
                "K parameter (typically 256) controls codebook size",
                "Use k-means to train codebook for each subvector",
                "Precompute distance lookup tables for query vector",
                "Consider SIMD for fast distance table lookups",
            ],
            "tags": ["distributed-systems", "vector-search", "pq", "compression"],
        },
    }
